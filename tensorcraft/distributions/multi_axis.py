...
class MultiAxisDist(Dist):
    ...
    __slots__ = ("__dims_mapping", "__block_sizes")
    ...
    def __init__(
        ...
        self.__dims_mapping: DimsMapType = tuple(dims_mapping_list)
        self.__block_sizes: tuple[int, ...] = tuple(block_sizes_list)
    ...
    def __eq__(self, other):
        ...
        if base_comp and isinstance(other, MultiAxisDist):
            return (self.__dims_mapping == other.__dims_mapping) and (
                self.__block_sizes == other.__block_sizes
            )
        else:
            return False
    ...
    def __str__(self):
        ...
        map_str = map_str[:-1] + "}"
        b_str = ",".join([str(x) if x != -1 else "∅" for x in self.__block_sizes])
    ...
    def isDistributed(self) -> bool:
        ...
        """
        dist = False
        for axis_mapping in self.__dims_mapping:
    ...
    def compatible(self, shape):  # noqa: D102
        ...
        if len(shape) != len(self.__dims_mapping) or len(shape) != len(
            self.__block_sizes
        ):
            log.debug(
                f"Tensor order and the number of dimensions in the distribution must match: {len(shape)} != {len(self.__dims_mapping)}"
            )
            return False

        # Check for axis to processor mesh compatibility
        for axis, (axis_size, assigned_dims, block_size) in enumerate(
            zip(shape, self.__dims_mapping, self.__block_sizes)
        ):
    ...
    def _distributeDim(self, dim: int, dim_size: int) -> torch.Tensor:
        mesh_dims_idx = self.__dims_mapping[dim]
        ...
        block_size = self.__block_sizes[dim]
    ...
    def _max_block_size_n_blocks(self, shape: torch.Size) -> tuple[int, int]:
        ...
        for i in range(len(shape)):
            mesh_dims_idx = self.__dims_mapping[i]
            if not mesh_dims_idx or len(mesh_dims_idx) == 0:
            ...
            axis_splits, _ = self.axisSplits(
                shape[i], self.__block_sizes[i], n_procs_axis
            )
    ...
    def allgather(
        ...
        if gather_dim is None:
            ...
            involved_dims = [
                ...
                for axis_map in self.__dims_mapping
                if len(axis_map) != 0
            ...
            tensor_axis = -1
            for axis, mappings in enumerate(self.__dims_mapping):
            ...
            if dist_axis_i != 0:
                new_block_sizes = list(self.__block_sizes)
            ...
            new_mapping = tuple(
                ...
                for axis_mappings in self.__dims_mapping
            )
    ...
    def split(
        ...
        if not self.isDistributed() or len(self.__dims_mapping[tensor_axis]) == 0:
            new_dims_mapping = tuple(
                mesh_dims if i == tensor_axis else mapping
                for i, mapping in enumerate(self.__dims_mapping)
            )
            new_block_size = tuple(
                block_size if i == tensor_axis else b_size
                for i, b_size in enumerate(self.__block_sizes)
            )
            ...
            new_block_size = self.__block_sizes
            new_dist = MultiAxisDist(self._pmesh, new_dims_mapping, new_block_size)
    ...
    def permute(
        ...
        for i, axis_dims in enumerate(self.__dims_mapping):
            if axis_dims:
        ...
        new_dim = MultiAxisDist(self._pmesh, new_dims_mapping, self.__block_sizes)
    ...
    def alltoall(
        ...
        int
            The number of involved processes in each sub communicator.
        """
        if not self.isDistributed() or len(self.__dims_mapping[from_tensor_axis]) == 0:
        ...
        new_dims_map_list = list(self.__dims_mapping)
        new_block_size_list = list(self.__block_sizes)

        block_size = (
            self.__block_sizes[from_tensor_axis] if block_size < 0 else block_size
        )

        if minor:
            moved_mesh_dims: tuple[int, ...] = (
                self.__dims_mapping[from_tensor_axis][-1],
            )
            relevant_procs = self._pmesh[moved_mesh_dims[0]]
            n_procs = relevant_procs

            # Find out new dims mapping
            new_from_t_axis_mapping = self.__dims_mapping[from_tensor_axis][:-1]
            new_to_t_axis_mapping = (
                self.__dims_mapping[to_tensor_axis] + moved_mesh_dims
            )
            ...
            new_from_t_axis_bs = (
                self.__block_sizes[from_tensor_axis] * n_procs
                if new_dims_map_list[from_tensor_axis] != ()
                else -1
            )

            to_t_axis_bs = (
                self.__block_sizes[to_tensor_axis]
                if self.__dims_mapping[to_tensor_axis] != ()
            ...
            moved_mesh_dims = self.__dims_mapping[from_tensor_axis]
    ...
    def change_block_size(
        ...
        moved_mesh_dims = self.__dims_mapping[tensor_axis]
        ...
        new_dist = MultiAxisDist(
            self.processorMesh, self.__dims_mapping, tuple(new_block_size_list)
        )
    ...
    def neighbours(
        ...
        for dim in range(len(self._pmesh)):
            is_free = True
            for i in range(len(shape)):
                if dim in self.__dims_mapping[i]:
                    is_free = False
                    break
        ...
        # permutation
        for axis in range(len(shape)):
            mapping = self.__dims_mapping[axis]
            if len(mapping) > 1:
        ...
        for source_axis in range(len(shape)):
            if len(self.__dims_mapping[source_axis]) != 0:
                for target_axis in range(len(shape)):
                    for b_size in [-1] + b_sizes:
                        ...
                        if len(self.__dims_mapping[source_axis]) > 1:
                            operation = (
